...
class LikeCommands(commands.Cog):
    ...
    def load_config(self):
        ...
        if os.path.exists(CONFIG_FILE):
            try:
                ...
                print(f"WARNING: Config file '{CONFIG_FILE}' is corrupt. Resetting to default.")
    ...
    @commands.hybrid_command(name="setlikechannel", description="Set channels where /like command is allowed.")
    @commands.has_permissions(administrator=True)
    @app_commands.describe(channel="The channel to allow/disallow /like command in.")
    async def set_like_channel(self, ctx: commands.Context, channel: discord.TextChannel):
        ...
        if channel_id_str in like_channels:
            ...
            self.save_config()
            await ctx.send(f"✅ Channel {channel.mention} has been **removed** from allowed /like channels.", ephemeral=True)
        else:
    ...
    @app_commands.describe(uid="Player UID (numbers only, minimum 6 digits).")
    async def like_command(self, ctx: commands.Context, uid: str):
        is_slash = ctx.interaction is not None

        if not await self.check_channel(ctx):
            msg = "This command is not available in this channel. Please use an authorized channel."
            if is_slash:
        ...
        if not uid.isdigit() or len(uid) < 6:
            await ctx.reply("Invalid UID. It must be numeric and at least 6 digits long.", mention_author=False, ephemeral=is_slash)
            return

        try:
            async with ctx.typing():
                # Use the bot's shared aiohttp session for requests
                async with self.bot.session.get(f"{self.api_host}/like?uid={uid}", headers=self.headers, timeout=10) as response:
